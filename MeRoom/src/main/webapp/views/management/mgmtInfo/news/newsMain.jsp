...
function new_odc(type) {
	if (type == 'new') { /* 신규등록 진행 */location.href="/mr/views/management/mgmtInfo/news/newsInsert.jsp" }
	if (type == 'back') { /* 목록으로 돌아가기 */location.href="/mr/getNewsList.do" }
	if (type == 'update') { /* 정보 수정 진행 */location.href="/mr/views/management/mgmtInfo/news/newsUpdate.jsp" }
}
...
function delete_news(idx) {
	if (confirm("해당 소식을 삭제하시겠습니까?")) {
		...
		objs.setAttribute('name', 'neIdx');
		objs.setAttribute('value', idx);
		form.appendChild(objs);
		form.setAttribute('method', 'post');
		form.setAttribute('action', '/mr/deleteNews.do');
...
function go_detail(type) {
	$.ajax({
		...
		success : function(data) {
			let html = "";
			html += "<div class='class_basic'><p>제목</p><div>" + data.neTitle + "</div></div>";
			html += "<div class='class_basic'><p>부제목</p><div>" + data.neSubtitle + "</div></div>";
			html += "<div class='class_basic'><p>작성일</p><div>" + data.neRegdate + "</div></div>";
			$(".left_box").html(html);
			
			html = "<div class='class_basic'><div><img src='/newsupload/" + data.neThumbnail + "'/></div></div>";
			$(".right_box").html(html);
			
			html = "<div class='class_info'><div>" + data.neContent + "</div></div>";
			$(".big_box").html(html);
			
			$("input[name=delete_button]").attr("onclick", "delete_news(" + data.neIdx + ")");
	...
	<main class="main_container">
		...
		<div class="right_container">
			<div class="right_main">
				<div class="main_box">
					<div class="odc_title">
						<div class="title_text">미룸 소식</div>
					</div>
					<!-- 전체 목록 테이블 -->
					<div class="odc_list">
						<table class="list_table">
							...
							<c:if test="${not empty newsList}">
								<c:forEach var="newsOne" items="${newsList }">
								<tr>
									<td>${newsOne.neIdx}</td>
									<td><a href="javascript:go_detail(${newsOne.neIdx })">${newsOne.neTitle}</a></td>
									<td>${newsOne.neRegdate}</td>
								</tr>
								</c:forEach>
							</c:if>
