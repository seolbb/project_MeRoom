...
function new_odc(type) {
	if (type == 'new') { /* 신규등록 진행 */location.href="/mr/views/management/mgmtInfo/promotion/promotionInsert.jsp" }
	if (type == 'back') { /* 목록으로 돌아가기 */location.href="/mr/getPromotionList.do" }
	if (type == 'update') { /* 정보 수정 진행 */location.href="/mr/views/management/mgmtInfo/promotion/promotionUpdate.jsp" }
}

function delete_promotion(idx) {
	if (confirm("해당 소식을 삭제하시겠습니까?")) {
		var form = document.createElement("form");
		var objs;
		objs = document.createElement('input');
		objs.setAttribute('type', 'text');
		objs.setAttribute('name', 'prIdx');
		objs.setAttribute('value', idx);
		form.appendChild(objs);
		form.setAttribute('method', 'post');
		form.setAttribute('action', '/mr/deletePromotion.do');
...
function go_detail(type) {
	$.ajax({
		...
		url : "/mr/getPromotion.do",
		success : function(data) {
			let html = "";
			html += "<div class='class_basic'><p>제목</p><div>" + data.prTitle + "</div></div>";
			html += "<div class='class_basic'><p>부제목</p><div>" + data.prSubtitle + "</div></div>";
			html += "<div class='class_basic'><p>작성일</p><div>" + data.prRegdate + "</div></div>";
			$(".left_box").html(html);
			
			html = "<div class='class_basic'><div><img src='/promotionupload/" + data.prThumbnail + "'/></div></div>";
			$(".right_box").html(html);
			
			html = "<div class='class_info'><div>" + data.prContent + "</div></div>";
			$(".big_box").html(html);
			
			$("input[name=delete_button]").attr("onclick", "delete_promotion(" + data.prIdx + ")");
	...
	<main class="main_container">
		...
		<div class="right_container">
			<div class="right_main">
				<div class="main_box">
					<div class="odc_title">
						<div class="title_text">미룸 프로모션</div>
					</div>
					<!-- 전체 목록 테이블 -->
					<div class="odc_list">
						<table class="list_table">
							...
							<c:if test="${not empty promotionList}">
								<c:forEach var="promotionOne" items="${promotionList }">
								<tr>
									<td>${promotionOne.prIdx}</td>
									<td><a href="javascript:go_detail(${promotionOne.prIdx })">${promotionOne.prTitle}</a></td>
									<td>${promotionOne.prRegdate}</td>
								</tr>
								</c:forEach>
							</c:if>
					...
					<div class="odc_searchbox">
						<input type="button" value="소식등록" onclick="new_odc('new');"
