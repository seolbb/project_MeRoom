...
function new_odc(type) {
	if (type == 'new') { /* 신규등록 진행 */location.href="/mr/views/management/mgmtInfo/event/eventInsert.jsp" }
	if (type == 'back') { /* 목록으로 돌아가기 */location.href="/mr/getEventListAdmin.do" }
	if (type == 'update') { /* 정보 수정 진행 */location.href="/mr/views/management/mgmtInfo/event/eventUpdate.jsp" }
}
...
function delete_event(idx) {
	if (confirm("해당 소식을 삭제하시겠습니까?")) {
		...
		objs.setAttribute('name', 'evIdx');
		objs.setAttribute('value', idx);
		form.appendChild(objs);
		form.setAttribute('method', 'post');
		form.setAttribute('action', '/mr/deleteEvent.do');
...
function go_detail(type) {
	$.ajax({
		...
		url : "/mr/getEventAdmin.do",
		success : function(data) {
			let html = "";
			html += "<div class='class_basic'><p>제목</p><div>" + data.evTitle + "</div></div>";
			html += "<div class='class_basic'><p>부제목</p><div>" + data.evSubtitle + "</div></div>";
			html += "<div class='class_basic'><p>작성일</p><div>" + data.evRegdate + "</div></div>";
			$(".left_box").html(html);
			
			html = "<div class='class_basic'><div><img src='/eventupload/" + data.evThumbnail + "'/></div></div>";
			$(".right_box").html(html);
			
			html = "<div class='class_info'><div>" + data.evContent + "</div></div>";
			$(".big_box").html(html);
			
			$("input[name=delete_button]").attr("onclick", "delete_event(" + data.evIdx + ")");
	...
	<main class="main_container">
		...
		<div class="right_container">
			<div class="right_main">
				<div class="main_box">
					<div class="odc_title">
						<div class="title_text">미룸 이벤트</div>
					</div>
					<!-- 전체 목록 테이블 -->
					<div class="odc_list">
						<table class="list_table">
							...
							<c:if test="${not empty eventList}">
								<c:forEach var="eventOne" items="${eventList }">
								<tr>
									<td>${eventOne.evIdx}</td>
									<td><a href="javascript:go_detail(${eventOne.evIdx })">${eventOne.evTitle}</a></td>
									<td>${eventOne.evRegdate}</td>
								</tr>
								</c:forEach>
							</c:if>
